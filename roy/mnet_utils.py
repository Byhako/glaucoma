...
def disc_crop(org_img, DiscROI_size, C_x, C_y, size_image=0):
    ...
    # si la alguna de las dimensiones esta fuera de la imagen, se ajusta el valor
    if size_image > 0:
        if crop_coord[0] < 0:
            crop_coord = np.array([0, DiscROI_size, C_y - tmp_size, C_y + tmp_size], dtype=int)
        if crop_coord[1] > size_image:
            crop_coord = np.array([511-DiscROI_size, 511, C_y - tmp_size, C_y + tmp_size], dtype=int)
        if crop_coord[2] < 0:
            crop_coord = np.array([C_x - tmp_size, C_x + tmp_size, 0, DiscROI_size], dtype=int)
        if crop_coord[3] > size_image:
            crop_coord = np.array([C_x - tmp_size, C_x + tmp_size, 511-DiscROI_size, 511], dtype=int)
