...
Original_vali_img_path = "C:\\Users\\USUARIO\\Desktop\\CADA\\1ORIGA\\Images_Square\\" #C:\Users\USUARIO\Desktop\CADA\1REFUGE\train\Images

Original_Mask_img_path = "C:\\Users\\USUARIO\\Desktop\\CADA\\1ORIGA\\Masks_Square\\" #C:\Users\USUARIO\Desktop\CADA\1REFUGE\


Image_save_path = "C:\\Users\\USUARIO\\Desktop\\CADA\\CADA_Tutorial_image3\\Images\\"#C:\Users\USUARIO\Desktop\CADA\CADA_Tutorial_image\Target_Test\Image
MaskImage_save_path = "C:\\Users\\USUARIO\\Desktop\\CADA\\CADA_Tutorial_image3\\Masks\\"#C:\Users\USUARIO\Desktop\CADA\CADA_Tutorial_image\Target_Train\Image
...
for lineIdx in range(0, len(file_train_list)):
    ...
    if is_only_image == False:
        ...
        plt.imshow(org_mask)# sale segmentado con las dos mascaras montada sin recortar y centrar
        plt.title('org_mask')
        plt.show()

        org_disc = org_mask < 255
        plt.imshow(org_disc) # sale todo morado
        plt.title('org_disc')
        plt.show()

        org_cup = org_mask == 0
        plt.imshow(org_cup) # sale amarillo el fondo y un circulo morado, segmentado el disco
        plt.title('org_cup')
        plt.show()

        temp_org_mask = resize(org_mask, (DiscSeg_size, DiscSeg_size))
        plt.imshow(temp_org_mask) # sale la imagen en mascara con el tamaño mas grande sin centrar disco y copa
        plt.title('temp_org_mask')
        plt.show()

        temp_org_disc = resize(org_disc, (DiscSeg_size, DiscSeg_size))
        plt.imshow(temp_org_disc) # sale la imagen en mascara  con el tamaño mas grande todo morado
        plt.title('temp_org_disc')
        plt.show()

        temp_org_cup = resize(org_cup, (DiscSeg_size, DiscSeg_size))
        plt.imshow(temp_org_cup)# sale la imagen en mascara  con el tamaño mas grande todo amarillo con el circulo morado.
    ...
    plt.title('temp_img')# sale un circulo amarillo con fondo morado.
    plt.show()
    ...
    for disc_idx, DiscROI_size in enumerate(ROI_size_list):
        ...
        plt.title('org_img_disc_region')# aparece recortada centrada con el tamaño de discroisize
        plt.show()

        org_mask_disc_region, err_coord, crop_coord = disc_crop(org_mask, DiscROI_size, C_x, C_y)
        plt.imshow(org_mask_disc_region)
        plt.title('org_mask_disc_region')  # aparece recortada centrada con el tamaño de discroisize
        plt.show()

        if is_only_image == False:
            org_mask_region, err_coord, crop_coord = disc_crop(org_cup, DiscROI_size, C_x, C_y)
            ...
            ROI_mask_result = np.array(BW_img(org_disc_region, 0.5), dtype=int) + np.array(BW_img(org_cup_region, 0.5),
                                                                                           dtype=int)
            #plt.imshow(ROI_mask_result)# aparecetodo en amarillo
            #plt.title('ROI_mask_result')
            #plt.show()

            ROI_mask_result = (255.0 / ROI_mask_result.max() * (ROI_mask_result - ROI_mask_result.min())).astype(
                np.uint8)
            ...
            #plt.imshow(ROI_mask_result) # aparecetodo en morado
            #plt.title('ROI_mask_result')
            #plt.show()
        ...
        if is_polar_coordinate:
            ...
            if is_only_image == False:
                ROI_mask_result = rotate(
                    cv2.linearPolar(ROI_mask_result, (DiscROI_size / 2, DiscROI_size / 2), DiscROI_size / 2,
            ...
            ROI_img_result = rotate(
                cv2.linearPolar(org_img_disc_region, (DiscROI_size / 2, DiscROI_size / 2), DiscROI_size / 2,
                                cv2.INTER_NEAREST + cv2.WARP_FILL_OUTLIERS), -90)
        else:
